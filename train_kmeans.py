    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                gaussians.max_radii2D = torch.where(
                        mask,
                        torch.maximum(gaussians.max_radii2D, radii),
...
def safe_add(tb_writer, method, *args, **kwargs):
    try:
        getattr(tb_writer, method)(*args, **kwargs)
    except Exception as e:
        print(f"[TensorBoard Warning] Failed to write {method}: {e}")
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
        ...
        safe_add(tb_writer, 'add_scalar', 'train_loss_patches/l1_loss', Ll1.item(), iteration)
        safe_add(tb_writer, 'add_scalar', 'train_loss_patches/total_loss', loss.item(), iteration)
        safe_add(tb_writer, 'add_scalar', 'iter_time', elapsed, iteration)
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if tb_writer and (idx < 5):
                        ...
                        safe_add(tb_writer, 'add_images', f"{config['name']}_view_{viewpoint.image_name}/render", image[None], global_step=iteration)
                        if iteration == testing_iterations[0]:
                            safe_add(tb_writer, 'add_images', f"{config['name']}_view_{viewpoint.image_name}/ground_truth", gt_image[None], global_step=iteration)
                ...
                if tb_writer:
                    ...
                    safe_add(tb_writer, 'add_scalar', f"{config['name']}/loss_viewpoint - l1_loss", l1_test, iteration)
                    safe_add(tb_writer, 'add_scalar', f"{config['name']}/loss_viewpoint - psnr", psnr_test, iteration)

        if tb_writer:
            # tb_writer.add_histogram("scene/opacity_histogram", scene.gaussians.get_opacity, iteration)
            # tb_writer.add_scalar('total_points', scene.gaussians.get_xyz.shape[0], iteration)
            safe_add(tb_writer, 'add_histogram', "scene/opacity_histogram", scene.gaussians.get_opacity, iteration)
            safe_add(tb_writer, 'add_scalar', 'total_points', scene.gaussians.get_xyz.shape[0], iteration)
        torch.cuda.empty_cache()
    return psnr_out
